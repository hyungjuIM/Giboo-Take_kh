...
function infoValidate(){
   ...
   // const regExp4 = /^[\w\.\_\-]{4,}@[\w\-\_]+(\.\w+){1,3}$/; // 이메일 유효성 검사 
    ...
    // 이름 유효성 검사
    if(memberName.value.length == 0){ // 미작성 시 : 이름을 입력해주세요.
        alert("이름을 입력해주세요.");
        memberName.focus();
        return false;
    }
    ...
    if(!regExp1.test(memberName.value)){ // 유효하지 않은 경우
        alert("이름은 영어/숫자/한글 2~10 글자 사이로 작성해주세요.");
        memberName.focus();
        return false;
    }
    ...
    // //이메일 유효성 검사
    // if(memberEmail.value.length == 0){ // 미작성 시 : 이메일을 입력해주세요.
    //     alert("이메일을 입력해주세요.");
    //     memberEmail.focus();
    //     return false;
    // }
...
function execDaumPostcode() {
    new daum.Postcode({
        oncomplete: function(data) {
            ...
            var memberAddress = ''; // 주소 변수
            var extraAddr = ''; // 참고항목 변수

            //사용자가 선택한 주소 타입에 따라 해당 주소 값을 가져온다.
            if (data.userSelectedType === 'R') { // 사용자가 도로명 주소를 선택했을 경우
                memberAddress = data.roadAddress;
            } else { // 사용자가 지번 주소를 선택했을 경우(J)
                memberAddress = data.jibunAddress;
            }
